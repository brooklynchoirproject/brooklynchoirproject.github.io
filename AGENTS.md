...
`npm run dev` starts a local server and prints the URL to open, usually
`http://localhost:5173/` (if that port is busy, Vite picks the next free one,
e.g. `5174`). Leave the command running in its terminal — Ctrl+C stops it. If
you already have a dev server running and just changed a file in `public/`
(like the video or poster), a hard refresh (Cmd+Shift+R) in the browser tab
